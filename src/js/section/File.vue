<template>
	<div class="st-up-box">
		<input type="hidden" v-bind="$attrs" v-model="val" />
		<div v-for="(info, idx) in files" class="st-up-item" :key="idx">
		...
		<label v-if="files.length<=0 || fieldConf.uploadConfig.multiple" class="st-up-item">
			...
			<input class="st-up-input" :accept="isImage?'image/*':'*/*'" type="file" @change="upload" />
		</label>
	</div>
</template>
<script>
...
<style scoped>
	...
	.st-up-progress-loaded{
		background: #409eff;
		color: #fff;
	}
	.st-up-progress-error{
		color: red;
	}
	.st-up-icon{
		font-size: 60px;
