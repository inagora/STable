...
export default {
	...
	watch: {
		...
		selIdxes(val) {
			if (val.length <= 0) {
				this.$el.querySelector('input').style.width =
					this.maxInputLength + 'px';
			} else {
				this.upadteInputWidth();
			}
		},
	...
	mounted() {
		...
		this.maxInputLength =
			this.$el.querySelector('.st-cbb-item-list').clientWidth -
			parseInt(computedStyle.paddingLeft) -
			parseInt(computedStyle.paddingRight);

		this.maxSelLabelWidth =
			this.$el.querySelector('.st-cbb-item-list').clientWidth - 4;
	...
	methods: {
		...
		hlDelPre() {
			...
			if (idx < 0) idx = this.selIdxes.length - 1;
			this.hlDelIndex = idx;
		},
		hlDelNext() {
			if (this.hlDelIndex < 0) {
			...
			let idx = this.hlDelIndex + 1;
			if (idx >= this.selIdxes.length) idx = 0;
			this.hlDelIndex = idx;
		},
		del(idx) {
